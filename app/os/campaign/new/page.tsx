...
export default function NewCampaign() {
  ...
  const { runInitialSynthesis, swarm, setCampaign } = useCampaignStore();
  const { logs, status } = swarm;
  ...
  // Logic to handle next step and trigger real synthesis
  const handleProceed = async () => {
    if (activeStep.isFinal) return;

    if (currentStep < STEPS.length - 1) {
      // If we're entering Phase 2: Orchestrating, save data and run it
  ...
  useEffect(() => {
    if (status === 'ready' && activeStep.isFinal) {
      const timer = setTimeout(() => router.push('/os/campaign/review'), 2000);
      return () => clearTimeout(timer);
    }
  }, [status, activeStep.isFinal, router]);
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="space-y-4 pt-4">
              {activeStep.isFinal ? (
                <TerminalStream logs={logs} status={status} />
              ) : (
                <>
      ...
      <footer className="p-6 pb-12 max-w-lg mx-auto w-full">
        {!activeStep.isFinal && !activeStep.options && (
          <button
            ...
            className="w-full py-5 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 text-white font-bold text-lg flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 transition-all shadow-2xl shadow-pink-500/20 disabled:opacity-50"
            disabled={status === 'synthesizing'}
          >
            {currentStep === 0 ? "Unlock Campaign" : (status === 'synthesizing' ? "Processing..." : "Next Step")}
        ...
        {activeStep.isFinal && (
          <div className="flex flex-col items-center gap-4 py-4">
             <div className="flex items-center gap-3 text-pink-400 font-medium capitalize">
               {status === 'synthesizing' ? <Clock className="w-5 h-5 animate-spin-slow" /> : <CheckCircle2 className="w-5 h-5" />}
               {status === 'synthesizing' ? 'Campaign Blueprint Pending' : 'Strategy Synthesized'}
             </div>
             <div className="w-full bg-slate-900 h-2 rounded-full overflow-hidden">
               <motion.div 
                 initial={{ width: 0 }} 
                 animate={{ width: status === 'ready' ? '100%' : '60%' }} 
...
function TerminalStream({ logs, status }: { logs: any[], status: string }) {
  ...
  return (
    <div 
      ref={scrollRef}
      className="w-full bg-slate-950/80 backdrop-blur-md border border-pink-500/20 rounded-xl p-4 font-mono text-xs overflow-y-auto h-48 shadow-inner shadow-pink-500/5 mt-8 flex flex-col"
    >
      {logs.map((log, i) => (
        <motion.div 
          key={log.id || i} 
        ...
        >
          ...
          <span className={log.type === 'success' ? 'text-green-400' : 'text-slate-300'}>
